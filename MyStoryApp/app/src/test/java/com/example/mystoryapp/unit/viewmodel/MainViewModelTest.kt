package com.example.mystoryapp.unit.viewmodel
...
import com.example.mystoryapp.utils.DataDummy.generateDummyNewStories
import com.example.mystoryapp.utils.DataDummy.generateDummyRequestLogin
import com.example.mystoryapp.utils.DataDummy.generateDummyRequestRegister
import com.example.mystoryapp.utils.DataDummy.generateDummyResponseLogin
...
class MainViewModelTest {
    ...
    @OptIn(ExperimentalPagingApi::class, ExperimentalCoroutinesApi::class)
    @Test
    fun `when successfully loading stories, ensure the data is not null`() = runTest {
        val noopListUpdateCallback = NoopListCallback()
        val dummyStory = generateDummyNewStories()
        val data = PagedTestDataSources.snapshot(dummyStory)
        val story = MutableLiveData<PagingData<ListStoryDetail>>()
        ...
        // Mock the getPagingStories function to return the prepared data
        `when`(mainViewModel.getPagingStories(token)).thenReturn(story)
    ...
    @OptIn(ExperimentalPagingApi::class, ExperimentalCoroutinesApi::class)
    @Test
    fun `when no stories are available, ensure an empty result is returned`() = runTest {
        val noopListUpdateCallback = NoopListCallback()
        val data = PagedTestDataSources.snapshot(listOf())
        val story = MutableLiveData<PagingData<ListStoryDetail>>()
        val token = "ini token"
        ...
        // Mock the getPagingStories function to return the prepared empty data
        `when`(mainViewModel.getPagingStories(token)).thenReturn(story)
